...
bool parseNextionDisplayModel(const char* model, HmiDisplayIdentity& out)
{
    ...
    if (toupper((unsigned char)model[0]) != 'N' ||
        toupper((unsigned char)model[1]) != 'X') return false;
    ...
    if (pos < len) {
        ...
        if (pos < len && !isSeparator_(model[pos])) {
            ...
            if (touch == 'R') out.touchType = HmiDisplayTouchType::Resistive;
            else if (touch == 'C') out.touchType = HmiDisplayTouchType::Capacitive;
            else if (touch == 'N') {
...
{
    return identity.compatibility[0] != '\0' && expectedCompatibility &&
           expectedCompatibility[0] != '\0' &&
                                  ...
                                  size_t versionOutLen)
{
    if (!filename || !compatibilityOut || compatibilityOutLen == 0U ||
        !versionOut || versionOutLen == 0U) return false;
    ...
    if (filenameLen <= prefixLen + extensionLen ||
        ...
        strcmp(filename + filenameLen - extensionLen, kArtifactExtension) != 0) return false;

    const char* versionSeparator = nullptr;
    for (const char* p = filename + prefixLen; p < filename + filenameLen - extensionLen; ++p) {
        if (*p == '-' && p[1] != '\0' && isdigit((unsigned char)p[1])) versionSeparator = p;
    }
    ...
    if (modelLen == 0U || modelLen >= HMI_DISPLAY_MODEL_TEXT_MAX ||
        versionLen == 0U || versionLen >= versionOutLen) return false;
    ...
    if (!parseNextionDisplayModel(model, parsed) ||
        ...
        parsed.touchType == HmiDisplayTouchType::Capacitive) return false;
