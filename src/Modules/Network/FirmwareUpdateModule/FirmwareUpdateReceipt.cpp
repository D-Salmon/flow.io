...
uint32_t appendByte_(uint32_t hash, uint8_t value) { return (hash ^ value) * kFnvPrime; }
...
{
    return state >= FirmwareUpdateReceiptState::Running && state <= FirmwareUpdateReceiptState::Failed;
}
} // namespace
...
bool firmwareUpdateReceiptIsValid(const FirmwareUpdateReceipt& receipt)
{
    return receipt.magic == kReceiptMagic &&
           receipt.schemaVersion == kReceiptSchemaVersion &&
           receipt.operationId != 0U && targetIsValid_(receipt.target) &&
           stateIsValid_(receipt.state) && receipt.checksum == checksum_(receipt);
}
