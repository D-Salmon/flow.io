(function () {
  ...
  window.FlowWebPages.updates = {
    ...
    function createUpgradeActionButton(row) {
      ...
      const coupledReleaseComponent = row && (row.key === 'flowios3' || row.key === 'spiffs');
      button.disabled = coupledReleaseComponent || !(entry && entry.endpoint && entry.url);
      button.title = button.disabled
        ? (coupledReleaseComponent
          ? 'Utilisez le package complet firmware + interface Web ci-dessus.'
    ...
    async function readStoredReleaseZip(file) {
      ...
      while (offset + 4 <= file.size) {
        ...
        if (signature !== 0x04034b50 || offset + 30 > file.size) throw new Error('structure ZIP invalide');
        ...
        if ((flags & 0x0009) !== 0 || method !== 0 || compressedSize !== uncompressedSize) {
          throw new Error('le package ZIP Flow.IO doit être non compressé');
        }
    ...
    function uploadReleaseBlob(url, blob, onProgress) {
      return new Promise((resolve, reject) => {
        ...
        const csrf = window.FlowWebCore && typeof window.FlowWebCore.csrfToken === 'function'
          ? window.FlowWebCore.csrfToken() : '';
        if (csrf) request.setRequestHeader('X-Flow-CSRF', csrf);
        ...
        request.onload = () => {
          ...
          if (request.status < 200 || request.status >= 300 || (payload && payload.ok === false)) {
            ...
            reject(new Error(detail || 'transfert refusé'));
            return;
          }
    ...
    async function installLocalRelease(file) {
      ...
      try {
        ...
        if (!manifest || manifest.format !== 1 || manifest.product !== 'Flow.IO' ||
          ...
          throw new Error('manifest de version incompatible');
        }
        const description = 'Flow.IO ' + manifest.version + '\nFirmware : ' + firmware.size +
          ' octets\nInterface web : ' + filesystem.size + ' octets';
        if (localReleaseSummary) localReleaseSummary.textContent = description.replace(/\n/g, ' · ');
        if (!confirm(description + '\n\nInstaller cette version puis redémarrer ?')) return;

        startUpgradeUiSession('release');
        setUpgradeMessage('Préparation de la version ' + manifest.version + '…');
        const started = await fetchOkJson('/api/upgrade/begin', {
          method: 'POST', cache: 'no-store', headers: { 'Content-Type': 'application/json' }, body: manifestText
        }, 'échec de préparation du package');
        transactionId = Number(started && started.transaction_id);
        if (!Number.isFinite(transactionId) || transactionId <= 0) throw new Error('transaction invalide');
        ...
        await uploadReleaseBlob('/api/upgrade/filesystem' + query, filesystem, (progress) => {
          setUpgradeMessage('Installation de l’interface web : ' + progress + '%');
        });
        await uploadReleaseBlob('/api/upgrade/firmware' + query, firmware, (progress) => {
          setUpgradeMessage('Installation du firmware : ' + progress + '%');
        });
        await fetchOkJson('/api/upgrade/commit' + query, { method: 'POST', cache: 'no-store' },
                          'échec de validation de la version');
        transactionId = 0;
        updateUpgradeUiSession({ phase: 'reboot', target: 'release',
          detail: 'Version installée. Flow.IO redémarre…', backendProgress: 100, awaitingReconnect: true });
        setUpgradeMessage('Version installée. Flow.IO redémarre…');
        enterUpgradeReconnectPhase();
      } catch (err) {
        if (transactionId > 0) {
          const abortOptions = window.FlowWebCore && typeof window.FlowWebCore.secureFetchOptions === 'function'
            ? window.FlowWebCore.secureFetchOptions({ method: 'POST', cache: 'no-store' })
            : { method: 'POST', cache: 'no-store' };
          await fetch('/api/upgrade/abort?transaction_id=' + encodeURIComponent(String(transactionId)), abortOptions).catch(() => {});
        }
      ...
      if (localReleaseFileInput) {
        localReleaseFileInput.addEventListener('change', () => {
          const file = localReleaseFileInput.files && localReleaseFileInput.files[0]
            ? localReleaseFileInput.files[0] : null;
          if (file) void installLocalRelease(file);
        });
      }
